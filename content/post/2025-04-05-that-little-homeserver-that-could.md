...
One thing to note: if you want to run tailscale and proton-vpn at the same time, you have to make sure that the proton-vpn is started before tailscale.

You can use `systemctl edit tailscale` to add the following lines to the `tailscale.service` file:

```ini
[Unit]
After=wg-quick@proton
```
...
- [Automatic Proton Drive/Mail backup to the server](https://snikt.net/blog/2025/04/27/homeserver-creating-local-proton-drive/mail-backups/).